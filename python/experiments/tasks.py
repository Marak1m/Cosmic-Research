...
class AINewsLetterTasks():
    ...
    def analyze_news_task(self, agent, context):
        return Task(
            description='Analyze the fetched news stories and generate markdown summaries for at least 5 well-formatted articles.',
            agent=agent,
            async_execution=True,
            context=context,
    ...
    def compile_newsletter_task(self, agent, context, save_func):
        return Task(
            description='Compile the analyzed news stories into a final newsletter format.',
            agent=agent,
            context=context,
            expected_output="""A complete newsletter in markdown format, with a consistent style and layout.
                ...
                **Why it matters:**...\\n\\n
            """,
            callback=save_func  # Ensure save_func is a properly defined function that handles saving the output
        )
